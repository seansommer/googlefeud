...
const keys = {
  sound: "googlefeud.soundEnabled.v1",
  effects: "googlefeud.effectsVolume.v1",
  music: "googlefeud.musicVolume.v1"
};
...
test("saved and legacy volumes survive the update; blocked storage is harmless", () => {
  ...
  const legacy = createAudio({ preferences: { "googlefued.musicVolume.v1": "0" } });
  assert.equal(legacy.sound.musicVolume, 0);
  assert.equal(legacy.stored.get(keys.music), "0");
  assert.equal(legacy.stored.has("googlefued.musicVolume.v1"), false);
